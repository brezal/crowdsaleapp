...
function stormpathLoginReg(first, last, email, clefID, req, res) {
  ...
  getAccount({ email: email, clefID: clefID }, req, function(account) {
    if (account) {
      ...
      request.post(account.href, {
        'auth': {
          'user': process.env.STORMPATHID,
          'pass': process.env.STORMPATHSECRET,
      ...
      if(accounts.items.length > 0) {
        ...
        req.app
          ...
          .getAccounts({email: data.email}, function(err, accounts) {
            ...
            if (accounts.items.length > 0 && accounts.items[0].directory.href!='https://api.stormpath.com/v1/directories/3XBbFH9AXEbElEpM3hk8Dx' && accounts.items[0].directory.href!='https://api.stormpath.com/v1/directories/3NGzzSCcS4Siy1n43fFMWH') {
